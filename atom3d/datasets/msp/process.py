...
def filter_dup_lines(lines):
    ...
    print
    '{} lines after getting rid of duplicates'.format(len(new_lines))
    return new_lines
...
def filter_multiple_muts(lines):
    ...
    for l in lines:
        if ',' in l[1] or ' ' in l[1].strip():
            ...
            single.append(l)
    print
    '{} lines after getting rid of multiple mutations'.format(len(single))
    return single
...
def filter_nonbinders(lines):
    ...
    for l in lines:
        if 'n.b.' not in l[6:10] and 'n.b' not in l[6:10] and '' not in l[6:10]:
            filtered.append(l)
    print
    '{} lines after getting rid of nonbinders'.format(len(filtered))
    return filtered
...
def get_labels(lines):
    ...
    for l in lines:
        ...
        counts[l[3]] += 1
    print
    '{} mutations out of {} were increasing'.format(sum(labels), len(labels))
    for loc in counts:
        print
        '{} in location: {}'.format(counts[loc], loc)
    print
    ''
    return labels
...
def process_pdbs(lines, input_dir, write_dir):
    if not os.path.exists(input_dir):
        print
        'The path {} does not exist'.format(input_dir)
        return
    written = set()
    counter = 0
    for l in lines:
        ...
        if tuple([pdb, chains1, chains2]) not in written:
            if not os.path.exists(os.path.join(write_dir, pdb + '_' + chains1 + '_' + chains2 + '.pdb')):
                ...
                print
                'Actually Wrote {} to {}'.format(pdb, write_dir)
            print
            '{} handled'.format(pdb)
        written.add(tuple([pdb, chains1, chains2]))
        counter += 1
    print
    '{} pdbs handled for {}'.format(counter, write_dir)
...
def main():
    ...
    print
    '{} total lines before filtering'.format(len(lines))
