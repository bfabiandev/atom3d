...
def convert_hdf5_to_npz(in_dir_name, out_dir_name, datatypes=None, shuffle=False,
    ...
    ds_tr = MoleculesDataset(tr_env_fn, shuffle=shuffle, max_num_atoms=max_num_atoms,
                             num_sampled_shards=int(num_sampled_shards_tr), name='training')
    ds_va = MoleculesDataset(va_env_fn, shuffle=False, max_num_atoms=max_num_atoms,
                             num_sampled_shards=int(num_sampled_shards_va), name='validation')
    ds_te = MoleculesDataset(te_env_fn, shuffle=False, max_num_atoms=max_num_atoms,
                             num_sampled_shards=int(num_sampled_shards_te), name='test')
    ...
    print(
        'Training: %i molecules. Validation: %i molecules. Test: %i molecules.' % (len(ds_tr), len(ds_va), len(ds_te)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--numshards_tr', dest='numshards_tr', type=int, default=10,
                        help='number of shards to sample for training')
    parser.add_argument('--numshards_va', dest='numshards_va', type=int, default=4,
                        help='number of shards to sample for validation')
    parser.add_argument('--numshards_te', dest='numshards_te', type=int, default=5,
                        help='number of shards to sample for testing')
    ...
    ds_tr, ds_va, ds_te = convert_hdf5_to_npz(args.in_dir, args.out_dir, datatypes=cormorant_datatypes,
                                              max_num_atoms=args.maxnumat,
                                              num_sampled_shards_tr=args.numshards_tr,
                                              num_sampled_shards_va=args.numshards_va,
                                              num_sampled_shards_te=args.numshards_te)
