...
def generate_split(excl_uncharacterized=True, excl_rdkitfails=True, out_dir_name='.', seed=42):
    ...
    test_indices, vali_indices, train_indices = splits.random_split(num_molecules, vali_split=0.1, test_split=0.1,
                                                                    random_seed=seed, exclude=exclude)
    print('Training: %i molecules. Validation: %i molecules. Test: %i molecules.' % (
...
if __name__ == "__main__":
    ...
    generate_split(excl_uncharacterized=args.excl_uncharacterized, excl_rdkitfails=args.excl_rdkitfails,
